...
@router.get('/birthday_date/', response_model=List[ContactResponse])
async def search_by_birthday(db: AsyncSession = Depends(get_db)):
    contacts = await repositories_contacts.search_by_birthday(db)
    if contacts is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="NOT FOUND")
    return [ContactResponse(**contact.__dict__) for contact in contacts]
